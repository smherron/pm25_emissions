...
## Find data that belongs to vehicles then subset for Balitmore City
...
baltimore_v_agg <- aggregate(Emissions ~ year, baltimore_vehicles, sum) 




png("plot5.png", width = 480, height = 480, bg = NA)
ggplot(baltimore_v_agg, aes(x = year, y = Emissions)) +
  geom_bar(stat = "identity", fill = "royalblue3") +
  labs(title = "Total Emissions for Vehicles in Baltimore City",
       # subtitle = "By Type",
       x = "Year",
